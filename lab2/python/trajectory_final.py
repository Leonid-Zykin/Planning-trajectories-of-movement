...
def generate_correct_trajectory():
    ...
    # Поворот на угол α (как показано на изображении)
...
def plot_correct_trajectory():
    """Построение правильной траектории"""
    
    # Генерация траектории
    t, x_ref, y_ref, theta_ref, vx_ref, vy_ref, omega_ref = generate_correct_trajectory()
    ...
    ax1.set_xlabel('X (м)')
    ax1.set_ylabel('Y (м)')
    ax1.set_title('Правильная траектория: прямая НАРУЖУ, полукруг R₂', fontsize=14)
    ax1.legend()
    ax1.grid(True)
    ax1.axis('equal')
    ...
    info_text = """Правильная траектория:
1. Окружность R₁=7м, δ=2π рад (снизу вверх, против часовой стрелки)
2. Поворот α=π/6 рад + прямая НАРУЖУ t=6с
3. ПОЛУКРУГ R₂=3м, π рад (по часовой стрелке)"""
    ...
    plt.savefig('../images/task1/trajectory_variant5_final.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    print("✅ Правильная траектория сохранена: trajectory_variant5_final.png")
    print(f"📊 Параметры траектории:")
    print(f"   Сегмент 1: R₁={7.0}м, δ={2*np.pi:.2f} рад, время={7.0*2*np.pi/1.0:.1f}с")
    print(f"   Сегмент 2: α={np.pi/6:.2f} рад, прямая НАРУЖУ, время={6.0}с")
...
if __name__ == "__main__":
    print("🚀 Генерация правильной траектории: прямая НАРУЖУ, полукруг R₂")
    plot_correct_trajectory()
