...
С правильной траекторией: прямая НАРУЖУ, полный круг R₂
"""
...
class FinalThreeWheelController:
    """Финальный контроллер для трехколесного мобильного робота"""
    ...
    def __init__(self):
        ...
        # Оптимизированные параметры контроллеров
        self.kp1, self.kp2 = 8.0, 8.0  # Высокие коэффициенты пропорционального управления
        self.kd1, self.kd2 = 4.0, 4.0  # Высокие коэффициенты дифференциального управления
        self.ki1, self.ki2 = 2.0, 2.0  # Высокие коэффициенты интегрального управления
    ...
    def final_static_controller(self, state, ref_state, ref_vel):
        """Финальная статическая линеаризация"""
        x, y, theta, beta_s1, beta_s2 = state
        ...
        try:
            ...
            # Адаптивные коэффициенты с улучшенной логикой
            error_norm = np.sqrt(e_x**2 + e_y**2)
            kp_adaptive = self.kp1 * (1 + 0.2 * error_norm)
        ...
        zeta = np.array([
            self.kp1 * e_theta + 0.8 * e_x,
            self.kp2 * e_theta - 0.8 * e_y
        ])
        
        return eta, zeta
    
    def final_dynamic_controller(self, state, ref_state, ref_vel, ref_acc, dt):
        """Финальная динамическая линеаризация с улучшенным ПИД-управлением"""
        x, y, theta, beta_s1, beta_s2 = state
        x_ref, y_ref, theta_ref = ref_state
        vx_ref, vy_ref, omega_ref = ref_vel
        ...
        # Производные ошибок (улучшенная аппроксимация)
        ...
        # Финальная динамическая линеаризация
        try:
            R_Sigma = R.T @ Sigma
            R_Sigma_pinv = np.linalg.pinv(R_Sigma)
            ...
            # Адаптивные коэффициенты с улучшенной логикой
            error_norm = np.sqrt(e_x**2 + e_y**2)
            kp_adaptive = self.kp1 * (1 + 0.1 * error_norm)
            kd_adaptive = self.kd1 * (1 + 0.05 * np.sqrt(de_x**2 + de_y**2))
            
            # Улучшенное ПИД-управление
            pid_term = np.array([
...
def generate_final_reference_trajectory():
    """Генерация финальной опорной траектории"""
...
def simulate_final_controllers():
    """Симуляция с финальными контроллерами"""
    
    # Создание контроллеров
    static_controller = FinalThreeWheelController()
    dynamic_controller = FinalThreeWheelController()
    
    # Генерация опорной траектории
    t_ref, x_ref, y_ref, theta_ref, vx_ref, vy_ref, omega_ref, ax_ref, ay_ref, alpha_ref = generate_final_reference_trajectory()
    ...
    t_sim = np.arange(0, 70, dt)  # Увеличено время для полного круга
    ...
    for i in range(1, len(t_sim)):
        ...
        eta, zeta = static_controller.final_static_controller(current_state, ref_state, ref_vel)
    ...
    for i in range(1, len(t_sim)):
        ...
        eta, zeta = dynamic_controller.final_dynamic_controller(current_state, ref_state, ref_vel, ref_acc, dt)
...
def plot_final_comparison():
    """Построение финального сравнения контроллеров"""
    
    # Симуляция
    t_sim, states_static, states_dynamic, t_ref, x_ref, y_ref, theta_ref = simulate_final_controllers()
...
if __name__ == "__main__":
    print("🚀 Симуляция финальных контроллеров для правильной траектории")
