...
def simple_static_controller(state, ref_state, ref_vel, kp=1.0, kd=0.5):
    """
    Простой статический контроллер
    """
...
def simple_dynamic_controller(state, ref_state, ref_vel, ref_acc, kp=1.0, kd=0.5, ki=0.1, integral_errors=None):
    """
    Простой динамический контроллер с интегральным управлением
    """
...
def simulate_robot_tracking(x0, t, ref_trajectory, controller_type='static', kp=1.0, kd=0.5, ki=0.1):
    """
    ...
    for i in range(1, n):
        ...
        if controller_type == 'static':
            u = simple_static_controller(state, ref_state, ref_vel, kp, kd)
        else:
            u, integral_errors = simple_dynamic_controller(state, ref_state, ref_vel, ref_acc, 
...
def plot_comparison_results(t, x_ref, y_ref, theta_ref, 
                          x_static, y_static, theta_static,
                          x_dynamic, y_dynamic, theta_dynamic,
                          title="Сравнение методов линеаризации"):
    """Построение графиков сравнения результатов"""
...
def main():
    from robot_model_fixed import generate_trajectory
    ...
    states_static = simulate_robot_tracking(x0, t, ref_trajectory, 'static', kp=1.0, kd=0.5)
    
    # Симуляция динамической линеаризации
    states_dynamic = simulate_robot_tracking(x0, t, ref_trajectory, 'dynamic', kp=1.0, kd=0.5, ki=0.1)
    ...
    # Построение графиков
    fig = plot_comparison_results(t, x_ref, y_ref, theta_ref,
                                x_static, y_static, theta_static,
                                x_dynamic, y_dynamic, theta_dynamic)
    ...
    fig.savefig(os.path.join(output_dir, "controllers_comparison.png"), dpi=300, bbox_inches='tight')
    plt.close()
    
    # Сохранение данных
    np.savez(os.path.join(output_dir, "controllers_results.npz"),
    ...
    print(f"Результаты сравнения контроллеров сохранены в {output_dir}")
