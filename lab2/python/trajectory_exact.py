...
Сегмент 2: Поворот α + прямая линия
Сегмент 3: Окружность R2 (по часовой стрелке)
"""
...
def generate_exact_trajectory():
    """Генерация точной траектории согласно изображению"""
...
def plot_exact_trajectory():
    """Построение точной траектории согласно изображению"""
    
    # Генерация траектории
    t, x_ref, y_ref, theta_ref, vx_ref, vy_ref, omega_ref = generate_exact_trajectory()
    ...
    ax1.set_xlabel('X (м)')
    ax1.set_ylabel('Y (м)')
    ax1.set_title('Точная траектория согласно изображению (вариант 5)', fontsize=14)
    ax1.legend()
    ax1.grid(True)
    ax1.axis('equal')
    ...
    info_text = """Точная траектория согласно изображению:
...
3. Окружность R₂=3м (по часовой стрелке)"""
    ...
    plt.savefig('../images/task1/trajectory_variant5_exact.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    print("✅ Точная траектория сохранена: trajectory_variant5_exact.png")
...
if __name__ == "__main__":
    print("🚀 Генерация точной траектории согласно изображению")
    plot_exact_trajectory()
