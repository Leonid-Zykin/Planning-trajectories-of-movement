...
class StaticFeedbackLinearization:
    """
    Статическая линеаризация обратной связи для мобильного робота
    """
    
    def __init__(self, robot, kp=2.0, kd=1.0):
        self.robot = robot
        self.kp = kp  # коэффициент пропорционального управления
        self.kd = kd  # коэффициент дифференциального управления
        
    def control_law(self, state, ref_state, ref_vel):
        """
        Закон управления со статической линеаризацией
        ...
        # Линеаризованные управляющие сигналы
        u1 = vx_ref + self.kp * ex + self.kd * evx
        u2 = vy_ref + self.kp * ey + self.kd * evy
        u3 = omega_ref + self.kp * etheta + self.kd * eomega
        
        # Преобразование в управляющие сигналы колес (4 колеса)
        u_wheels = np.array([u1, u2, u3, 0.0])  # четвертое колесо не используется
        
        return u_wheels
    
    def simulate_tracking(self, x0, t_span, ref_trajectory):
        """
        Симуляция слежения за траекторией
        """
        def rhs(state, t):
            # Получение эталонной траектории в момент времени t
            ref_state, ref_vel = ref_trajectory(t)
            
            # Вычисление управления
            u = self.control_law(state, ref_state, ref_vel)
...
def create_reference_trajectory(x_traj, y_traj, theta_traj, t):
    """
    ...
    def ref_trajectory(t):
        ...
        return ref_state, ref_vel
    
    return ref_trajectory

def plot_tracking_results(t, x_ref, y_ref, theta_ref, x_actual, y_actual, theta_actual, 
                         title="Результаты слежения за траекторией"):
    """Построение графиков результатов слежения"""
...
def main():
    from robot_model_fixed import FourWheelMobileRobot, generate_trajectory
    ...
    # Создание функции эталонной траектории
    ref_trajectory = create_reference_trajectory(x_ref, y_ref, theta_ref, t)
    
    # Создание контроллера
    controller = StaticFeedbackLinearization(robot, kp=2.0, kd=1.0)
    ...
    # Построение графиков
    fig = plot_tracking_results(t, x_ref, y_ref, theta_ref, 
                              x_actual, y_actual, theta_actual,
                              "Статическая линеаризация обратной связи")
    
    # Сохранение результатов
    output_dir = "/home/leonidas/projects/itmo/Planning-trajectories-of-movement/lab2/images/task3"
    os.makedirs(output_dir, exist_ok=True)
    
    fig.savefig(os.path.join(output_dir, "static_linearization.png"), dpi=300, bbox_inches='tight')
    plt.close()
    
    # Сохранение данных
    np.savez(os.path.join(output_dir, "static_results.npz"),
    ...
    print(f"Результаты статической линеаризации сохранены в {output_dir}")
