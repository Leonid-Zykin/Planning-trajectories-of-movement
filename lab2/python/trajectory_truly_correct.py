...
Сегмент 2: Поворот α + прямая линия НАРУЖУ от круга (в противоположном направлении)
Сегмент 3: ПОЛУКРУГ R2 (π радиан, по часовой стрелке)
"""
...
def generate_truly_correct_trajectory():
    """Генерация действительно правильной траектории согласно изображению"""
    ...
    t3 = R2 * np.pi / v  # Время движения по ПОЛУКРУГУ (π радиан)
    ...
    # СЕГМЕНТ 2: Поворот на α и движение по прямой НАРУЖУ (в противоположном направлении)
    mask2 = (t > t1) & (t <= t1 + t2)
    t2_local = t[mask2] - t1
    ...
    # КЛЮЧЕВОЕ ИСПРАВЛЕНИЕ: Прямая должна идти НАРУЖУ от круга
    # Направление наружу - это направление от центра круга к точке выхода
    outward_direction = np.arctan2(y_end1 - yc1, x_end1 - xc1)
    # Поворот на угол α в направлении наружу
    ...
    # Центр окружности R2 находится на расстоянии R2 от прямой в направлении поворота
    center_offset = R2 * np.array([-np.sin(theta_turn), np.cos(theta_turn)])
    
    xc2 = x_start2 + center_offset[0]
    yc2 = y_start2 + center_offset[1]
    
    # Угол для второй окружности (ПОЛУКРУГ, по часовой стрелке)
...
def plot_truly_correct_trajectory():
    """Построение действительно правильной траектории"""
    
    # Генерация траектории
    t, x_ref, y_ref, theta_ref, vx_ref, vy_ref, omega_ref = generate_truly_correct_trajectory()
    ...
    # Сегмент 3: полукруг R2
    mask3 = t > 7.0 * 2 * np.pi / 1.0 + 6.0
    ax1.plot(x_ref[mask3], y_ref[mask3], 'b-', linewidth=3, alpha=0.8, label='Сегмент 3: ПОЛУКРУГ R₂=3м')
    ...
    ax1.set_xlabel('X (м)')
    ax1.set_ylabel('Y (м)')
    ax1.set_title('Действительно правильная траектория: прямая НАРУЖУ, полукруг R₂', fontsize=14)
    ax1.legend()
    ax1.grid(True)
    ax1.axis('equal')
...
3. ПОЛУКРУГ R₂=3м, π рад (по часовой стрелке)"""
    ...
    plt.savefig('../images/task1/trajectory_variant5_truly_correct.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    print("✅ Действительно правильная траектория сохранена: trajectory_variant5_truly_correct.png")
    ...
    print(f"   Сегмент 3: R₂={3.0}м, ПОЛУКРУГ π рад, время={3.0*np.pi/1.0:.1f}с")
...
if __name__ == "__main__":
    print("🚀 Генерация действительно правильной траектории: прямая НАРУЖУ от круга, полукруг R₂")
    plot_truly_correct_trajectory()
