...
def plot_curvature_comparison_c2_methods(t_natural, curvature_natural, t_notaknot, curvature_notaknot, 
                                       t_clamped, curvature_clamped, filename):
...
def main():
    ...
    plot_curvature_comparison_c2_methods(t_natural, curvature_natural, t_notaknot, curvature_notaknot,
                                       t_clamped, curvature_clamped,
    ...
    # Проверяем кривизну на концах
    print("\nКривизна на концах (первые и последние 10 точек):")
    print(f"Natural - начало: {np.mean(curvature_natural[:10]):.4f}, конец: {np.mean(curvature_natural[-10:]):.4f}")
    print(f"Not-a-knot - начало: {np.mean(curvature_notaknot[:10]):.4f}, конец: {np.mean(curvature_notaknot[-10:]):.4f}")
    print(f"Clamped - начало: {np.mean(curvature_clamped[:10]):.4f}, конец: {np.mean(curvature_clamped[-10:]):.4f}")
