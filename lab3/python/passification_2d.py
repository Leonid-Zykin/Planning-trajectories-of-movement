...
class PassificationControl2D:
    """
    Алгоритм стабилизации 2D-траекторий методом пассификации.
    """

    def __init__(self, m=1.0, k=180.0, gamma=200.0, k_tau=15.0, d_tau=6.0,
        ...
        self.grad_phi_funcs = [
            ...
            lambda x, y: np.array([2*(x-2)/9, 2*(y-1)/4]),
            lambda x, y: np.array([-x, 1])
        ]
    ...
    def control_law(self, state, segment, segment_idx, s_star):
        """
        Закон пассификации для 2D случая.
        u = -γ·y·n - k·ẏ·n + s*τ
        где y = φ(x, y) - выходная переменная
        ẏ = ∇φ·v - производная выходной переменной
        """
        ...
        # Выходная переменная для пассификации
        phi_func = self.phi_funcs[segment_idx]
        y_output = phi_func(x, y)

        # Градиент функции траектории
        grad_phi_func = self.grad_phi_funcs[segment_idx]
        grad_phi = grad_phi_func(x, y)
        grad_phi_norm = np.linalg.norm(grad_phi)
        if grad_phi_norm > 1e-6:
            n_phi = grad_phi / grad_phi_norm
        else:
            n_phi = n

        # Производная выходной переменной: ẏ = ∇φ·v
        dy_dt = np.dot(grad_phi, vel)

        # Закон пассификации
        u_normal = -self.gamma * y_output * n_phi - self.k * dy_dt * n_phi

        # Касательное управление
        v_tau = np.dot(vel, tau)
        tangential_error = s_effective - v_tau
        u_tangential = self.k_tau * tangential_error * tau - self.d_tau * v_tau * tau
    ...
    def dynamics(self, state, t, segment, segment_idx, s_star):
        u, gamma_dot = self.control_law(state, segment, segment_idx, s_star)
        x_dot = state[2]
        y_dot = state[3]
        vx_dot = u[0] / self.m
        vy_dot = u[1] / self.m
    ...
    def simulate_segment(self, initial_state, segment, segment_idx, s_star, dt, max_time, phase_id):
        t_local = np.arange(0.0, max_time, dt)

        def rhs(state, t):
            return self.dynamics(state, t, segment, segment_idx, s_star)
    ...
    def simulate_trajectory(self, x0, t_grid, s_star):
        ...
        for idx, segment in enumerate(self.segments):
            ...
            t_local, sol, phase_vec = self.simulate_segment(
                current_state, segment, idx, s_star, dt, approx_time, idx + 1
            )
...
def plot_passification_2d(controller, time_hist, sol, phases, s_star):
    ...
    ax1.set_title(f'Траектория (пассификация, s* = {s_star})')
    ax1.set_xlabel('X')
    ax1.set_ylabel('Y')
    ax1.axis('equal')
    ax1.grid(True)
    ...
    ax3.set_title('Ошибки стабилизации (пассификация)')
    ...
    ax4.set_title('Управляющие воздействия (пассификация)')
...
def main():
    controller = PassificationControl2D()
    x0 = np.array([2.0, 0.0, 0.0, 0.0])
    t_grid = np.linspace(0, 40, 4000)
    speeds = [1.0, 3.0, 5.0]

    for s_star in speeds:
        print(f"Симуляция 2D пассификации для s* = {s_star}")
        time_hist, sol, phases = controller.simulate_trajectory(x0, t_grid, s_star)
        fig, phi_errors = plot_passification_2d(controller, time_hist, sol, phases, s_star)
        ...
        output_dir = "/home/leonidas/projects/itmo/Planning-trajectories-of-movement/lab3/images/task4"
        os.makedirs(output_dir, exist_ok=True)
        fig.savefig(os.path.join(output_dir, f"passification_2d_s{s_star}.png"), dpi=300, bbox_inches='tight')
        plt.close(fig)
